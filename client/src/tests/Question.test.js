...
test("Submit question form", async () => {
  render(<App />);
  const linkElement = await screen.getByRole('button', {name: /Submit Parameters/i});
  userEvent.click(linkElement);
  const result = await screen.getByTestId('recommendations');
  expect(result).toBeInTheDocument();
});

// test("Fill input in question form, then submit", async () => {
//   render(<App />);
//   const input1 = await screen.getByTestId('input1')
//   input1.setSelectionRange(0, 1)
//   userEvent.type(input1, '{backspace}75')
//   expect(input1).toHaveValue('75')

//   const input2 = await screen.getByTestId('input2')
